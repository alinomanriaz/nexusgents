...
const Page = () => {
    ...
    const onsubmithandler = async () => {
        ...
        try {
            ...
            console.log('data send post api not working', error)
        }
    }
    ...
    return (
        <>
            <div className=' relative w-full h-screen flex justify-center items-center'>
                ...
                <div className='w-[30%] relative pb-24 bg-blue-950/50 rounded-xl p-8 flex flex-col justify-center items-center space-y-6'>
                    <div className='capitalize text-blue-800 text-2xl font-semibold '>Login account</div>
                    <form action="" onSubmit={onsubmithandler} className='flex flex-col justify-center items-center space-y-4 w-full' >
                        ...
                        <Link className='text-blue-700 px-2 text-start text-sm w-full' href={'/signin'}>Forget password?</Link>
                        <button disabled={loading} type='submit' className='w-full bg-blue-800 rounded-lg py-2'>{loading ? <span className="loader "></span> : <>Sign In</>}</button>
                        {/* Error Message */}
                        {error && <div className="text-red-500 text-sm">{error}</div>}
                        <div className='text-white/40 text-sm py-2 text-center rounded-b-lg bottom-0 w-full absolute bg-gray-800/80 '>Dont have an account?<Link className='text-blue-700 px-2' href={'/signup'}>Sign Up</Link></div>
                    </form>
                </div>
            </div>
        </>
