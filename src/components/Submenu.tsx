...
const Submenu: React.FC<SubmenuServicesProps> = ({ close }) => {
  return (
    <div
      className="hidden md:flex fixed inset-0 top-16 justify-center"
      onMouseLeave={close}
    >
      <motion.div
        ...
        className="bg-white dark:bg-[#0f172a] w-[90%] h-fit max-w-6xl rounded-lg shadow-xl ring-1 ring-gray-200 dark:ring-gray-700 p-6"
      >
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
          {services.map((item, idx) => (
            <Link
              key={idx}
              href={item.href}
              className="group p-4 rounded-lg hover:bg-gray-50 dark:hover:bg-[#0f172a] dark:hover:ring-gray-600/20 dark:hover:ring-1 transition"
            >
