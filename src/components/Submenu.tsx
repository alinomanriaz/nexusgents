...
import { BiCodeAlt, BiMobile, BiBrush, BiRocket, BiObjectsHorizontalCenter } from 'react-icons/bi'
...
const services = [
  ...
  },
  {
    ...
    menu: [
      ...
      {
        submenu: 'Social Media Marketing',
        href: '/services/content-marketing/social-media-marketing'
      },
      {
            ...
            >
              ...
              </div>
              {
                item.menu?.map((data,index)=>(
                  <Link onClick={close} href={`${data.href}`} key={index} className="hover:border-l-3 border-blue-500 hover:pl-3 w-full text-sm text-gray-800 dark:text-gray-400 my-0.5 transition-all ease-in-out duration-100 pl-1">
                    {data.submenu}
                  </Link>
                ))
              }
